...
# Patterns for L1 token/spam heuristics (hex-encoded ASCII in witness)
_SPAM_HEX_PATTERNS = (
...
def get_tapscript_hex(witness: list[str]) -> str | None:
    if not witness:
        return None
    annex = has_annex(witness)
    if not infer_taproot_script_path(witness):
        return None
    cb_idx = len(witness) - (2 if annex else 1)
    script_idx = cb_idx - 1
    if script_idx < 0:
        return None
    return witness[script_idx]
...
def has_op_false_op_if_envelope(script_hex: str) -> bool:
    """Detect Ordinals inscription envelope: OP_0/OP_FALSE ... OP_IF ... OP_ENDIF."""
    buf = hex_to_bytes(script_hex)
    if len(buf) < 3:
        return False
    # Look for 0x00 0x63 (OP_FALSE OP_IF) or 0x00 ... 0x63 pattern
...
def detect_inscription_in_witness(witness: list[str] | None) -> bool:
    if not witness:
        return False
    annex = has_annex(witness)
    # Script-path: tapscript is second-to-last (excluding annex)
    if len(witness) > (2 if annex else 1):
        ...
        if script_idx >= 0:
            ...
            if has_op_false_op_if_envelope(tapscript):
                return True
    # Also scan all witness items for envelope pattern
    for item in witness:
...
def decode_coinbase_tag(coinbase_hex: str) -> str:
    ...
    # Pool tags usually contain slashes: /Foundry USA/, /ViaBTC/Mined/, etc.
    pool_runs = [r.strip() for r in runs if "/" in r and len(r.strip()) >= 5]
    if pool_runs:
        return max(pool_runs, key=len)[:40]

    # Fallback: longest printable run, skip very short noise
