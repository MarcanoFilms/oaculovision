...
def format_address_inspection(ins: AddressInspection) -> str:
    lines: list[str] = [
        f"[bold #ffd700]Address[/]  {ins.address}",
        "",
    ]
    if ins.error:
    ...
    lines.extend([
        ...
        "[dim]Press [bold]a[/] on a tx address to inspect · [bold]c[/] copy[/]",
    ])
    return "\n".join(lines)
...
class AddressService:
    ...
    def inspect_address(self, address: str) -> AddressInspection:
        ...
        try:
            validation = self.client.validate_address(address)
        ...
        try:
            scan = self.client.scantxoutset_address(
                address,
    ...
    def _scan_mempool_for_address(self, address: str) -> tuple[int, float]:
        ...
        try:
            mempool = self.client.get_raw_mempool(verbose=False)
        ...
        for txid in mempool[:limit]:
            try:
                tx = self.client.get_raw_transaction(str(txid), True)
            except BitcoinCLIError:
                continue
