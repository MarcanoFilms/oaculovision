"""Transaction fetch and deep analysis."""
...
from oraculovision.analysis.bip110 import TxAnalysis, analyze_transaction
from oraculovision.analysis.mempool_compose import categorize_transaction
from oraculovision.analysis.tx_flow import TxFlowSummary, TxIO, build_flow_summary, parse_inputs_from_tx
from oraculovision.config import AddressConfig
...
class TxInspectContext:
    """Optional hints when inspecting from Block Explorer or Mempool Glass."""
...
class TxInspection:
    ...
    fee_rate: float | None = None  # sat/vB
...
def parse_tx_query(raw: str) -> str:
    ...
    if not _TXID_RE.fullmatch(txid):
        raise TxQueryError(
            "Invalid txid — must be 64 hexadecimal characters"
        )
    return txid
...
class TxService:
    ...
    def __init__(
        self,
        client: NodeClient,
        config: AddressConfig | None = None,
    ) -> None:
        self.client = client
    ...
    def inspect_txid(
        ...
        *,
        ...
        try:
            mempool = self.client.get_raw_mempool(verbose=True)
    ...
    def _enrich_flow(self, tx: dict[str, Any]) -> TxFlowSummary:
        ...
        for io in inputs:
            ...
            if parent_txid is None or parent_vout is None:
                continue
            try:
                parent = self.client.get_raw_transaction(str(parent_txid), True)
            except BitcoinCLIError:
                lookups += 1
                continue
            lookups += 1
    ...
    def _resolve_raw_tx(
        ...
        for block_hash, label in attempts:
            try:
                tx = self.client.get_raw_transaction(
                    txid,
                    True,
                    block_hash=block_hash,
                )
    ...
    def _not_found_message(self, txid: str, ctx: TxInspectContext) -> str:
        ...
        try:
            ...
            if chain.get("pruned"):
                ...
                lines.append(
                    f"Your node is [bold]pruned[/] (prune height #{prune_h:,}). "
                    "Older confirmed txs are not stored unless you pass block context."
                )
                lines.append(
                    "Tip: inspect from Block Explorer (Enter on block → i on flagged tx) "
...
def format_inspection(ins: TxInspection) -> str:
    """Render inspection as markup text for the TUI."""
    a = ins.analysis
    lines: list[str] = []
    ...
    lines.extend([
        f"[bold #ffd700]Transaction[/]  {ins.txid}",
        "",
        f"[bold]Status[/]      "
    ...
    lines.append("[bold #ffd700]Spam signals[/]")
    if a.signals:
    ...
    if ins.partial:
        ...
        lines.extend([
            "",
            "[dim]Press [bold]a[/] on an address · [bold]c[/] copy · verified via your node[/]",
        ])
    return "\n".join(lines)